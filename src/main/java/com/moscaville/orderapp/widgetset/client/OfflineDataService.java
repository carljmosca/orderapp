...
public class OfflineDataService {
    ...
    public static void localStoreTicket(final CustomerOrder ticket) {
    ...
    public static List<CustomerOrder> getAndResetLocallyStoredTickets() {
        ArrayList<CustomerOrder> al = new ArrayList<CustomerOrder>();
        StorageMap s = new StorageMap(Storage.getLocalStorageIfSupported());
        String ticketCount = s.get(TICKETCOUNT_KEY);
        if (ticketCount != null) {
            ...
            for (int i = 0; i < c; i++) {
                ...
                CustomerOrder fromJSON = CustomerOrder.deserialize(json);
                al.add(fromJSON);
                s.remove(key);
            }
