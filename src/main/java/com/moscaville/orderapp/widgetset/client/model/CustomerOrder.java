...
public class CustomerOrder implements Serializable {
    private Topping location = new Topping();
    private Date timeStamp = new Date();
    private String registerPlateNumber;
    private Coating violation;
    private String area;
    ...
    public Topping getLocation() {
        return location;
    }

    public void setLocation(Topping location) {
    ...
    public Coating getViolation() {
        return violation;
    }

    public void setViolation(Coating violation) {
    ...
    public static CustomerOrder deserialize(final String str) {
        CustomerOrder result = null;
        if (str != null) {
            result = new CustomerOrder();
            String[] split = str.split(DELIMITER);
            result.setLocation(Topping.deserialize(split[0]));
            result.setTimeStamp(new Date(Long.parseLong(split[1])));
            result.setRegisterPlateNumber(split[2]);
            result.setViolation(Coating.valueOf(split[3]));
